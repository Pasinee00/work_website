    ...
    class Master extends Connection{
        ...
        public function save_approve($data){
            ...
            // ทำให้ของเก้่ามี Doc_use = N
                ...
                copy($_FILES["files_pdf2"]["tmp_name"],"../../../Master_doc/".iconv("UTF-8", "TIS-620",$ex[0]).".".$ex[1]);
                ...
                $sql .=", approve_date =".$this->chk_value($data[approve_date]);
                ...
                $sql .=", time ='".date("H:i:s")."'";
                $sql .=", Doc_use = 'Y'";
                $sql .=", file_pdf = ".$this->chk_value("Master_doc/".$ex[0].'.'.$ex[1]);
                $resultData = $this->conn->prepare($sql);
                $resultData->execute();
                // return $sql;
        ...
        public function view_master_doc($data){
            ...
            if($data[status_Doc] !=''){
                ...
                $sqlN .= " ORDER BY pis_db.tbl_branch.brn_order,
                                    master_doc.Doc_order";
            $arr = array();
            // $sqlN.= " AND website_db.tb_news.news_id ='".$data[news1_id]."'";
            $q_INM = $this->conn->prepare($sqlN);
            $q_INM->execute();
        ...
        public function send_to_approver($data){
            ...
            $row = $result->fetch(PDO::FETCH_ASSOC);
                ...
                $mail->FromName ="ขอให้ท่านอนุมัติเอกสารระบบ ISO 14001";
             
        
                $mail->AddAddress('isdp@toyotanont.com',1);
                $mail->AddAddress('dco@toyotanont.com',2);
                $mail->AddAddress($row[email_company],3);

                $mail->WordWrap = 50;	// Set word wrap to 50 characters
                $text ="เรียน คุณ ".$row[emp_name]." ".$row[sec_nameThai]." ".$row[brn_name];
                $sendsubject="=?utf-8?b?".base64_encode($text)."?=";
                ...
                $mail->Subject =$sendsubject; 
                    ...
                    $time=date("H:i:s");
                    $mail->Body = "
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; เรียน คุณ  ".$row[emp_name]." ".$row[sec_nameThai]." ".$row[brn_name]."<br /><br />
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; สืบเนื่องจากที่มีการแก้ไขเอกสาร  ".$row[Doc_code]." ".$row[Doc_Name]."<br />
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; และทางแผนกบริหารคุณภาพได้ดำเนินการแก้ไขแล้วเสร็จ จึงขอให้ท่านอนุมัติเอกสารดังกล่าวในระบบ Approve Center <br />
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; หรือที่ Link   http://10.2.1.233/approvecenter หรือ สำหรับภายนอก www.nontrcms.com/approvecenter<br />
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ภายในวันที่ : ".$this->DateThai2($row[approve_date])." เพื่อประกาศใช้เอกสารใน วันที่ ".$this->DateThai2($row[Due_date])."<br />
                <br />
